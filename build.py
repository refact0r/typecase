...
# ///
"""Build the font assets for index.html from fonts.toml.
...
def convert(family, installed):
    ...
    for f in faces:
        ...
        font.flavor = 'woff2'
        font.save(out)
        print(f'  {family}: {src.name} -> {out.relative_to(ROOT)}')
    for stale in set(dest.glob('*.woff2')) - wanted:
        print(f'  {family}: removing stale {stale.relative_to(ROOT)}')
        stale.unlink()
    return True
...
def font_face(family, f):
    lines = [f'font-family: "{family}";',
             f"src: url(\"{f['path'].relative_to(ROOT).as_posix()}\") format(\"woff2\");",
