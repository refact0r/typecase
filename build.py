...
Writes, all under site/:
  ...
  fonts.js                 the list the page renders, and the weights each font really has
...
def slider_weights(ranges):
    """The stops of the page's weight slider (100, 200 … 900) that fall inside any of these (min, max) ranges."""
    return [w for w in range(100, 1000, 100) if any(lo <= w <= hi for lo, hi in ranges)]


def google_weights(meta):
    wght = next((a for a in meta['axes'] if a['tag'] == 'wght'), None)
    if wght:
        return slider_weights([(wght['min'], wght['max'])])
    return slider_weights([(int(s.rstrip('i')),) * 2 for s in meta['fonts']])


def fontshare_weights(meta):
    # italics are numbered one above their upright (401 for 400); 1 and 2 are the variable styles
    static = [s['weight']['number'] for s in meta['styles'] if not s['is_variable'] and not s['is_italic']]
    if any(s['is_variable'] for s in meta['styles']):
        return slider_weights([(min(static), max(static))])
    return slider_weights([(w, w) for w in static])
...
def main():
    ...
    weights = {n: google_weights(google[n]) for n in from_google}
    weights |= {n: fontshare_weights(fontshare[n]) for n in from_fontshare}
    ...
    for n in sorted(self_hosted, key=str.lower):
        ...
        weights[n] = slider_weights([f['weight'] for f in files if not f['italic']])
    ...
    (SITE / 'fonts.js').write_text(
        ...
        + 'const WEIGHTS = {\n'
        + ''.join(f'  {json.dumps(n, ensure_ascii=False)}: {json.dumps(w)},\n' for n, w in sorted(weights.items()))
        + '};\n')
    if licenses:
